...
exports.handler = async (event, context,callback) => {
    ...
    try {
        let registros = await dynamo.scan({ TableName: "tabelateste" }).promise();
        
        //Limpar os Registros
        for (var x = 0; x < registros.Items.length; x++) {
            response = await  dynamo.delete({Key:registros.Items[x].particaoteste}).promise()
        }
        
        //Adicionar os novos registros
        for (var i = 0; i < produtosBestsellers.length; i++) {
            response = await dynamo.put({...produtosBestsellers[i],particaoteste:i}).promise();
        }
